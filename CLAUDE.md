...
е преносим към други репо-та — виж `HANDOFF_KIT.md`. Auto-pull-ът е **opt-in**:
работи само ако съществува `.claude/handoff-sync-on` (тук е включен). Без този
файл hook-ът само показва състоянието, без да дърпа — удобно при инсталиране
по време на работа.
